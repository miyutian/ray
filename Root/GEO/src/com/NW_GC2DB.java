...
public class NW_GC2DB {  
	...
	public void gctodb(ArrayList<String>gcList) throws  Exception{
	//public void gctodb() throws  Exception{
		//UnZip UZ = new UnZip();
		//ArrayList<String> gcList = UZ.UnZipgz();
		for (int i = 0; i<gcList.size();i++){
			if (gcList.get(i).indexOf("___________gc.shp") != -1){
			        String pathString = gcList.get(i);
				 	//ShapefileDataStore shpDataStore = null;  
			        int batchCount = 0;
			        //System.out.println(pathString);
			        ShapefileDataStore shpDataStore = new ShapefileDataStore(new File(pathString).toURL());  
	...
	public void nwtodb(ArrayList<String >nwList) throws Exception{
	//public void nwtodb() throws Exception{
		//UnZip UZ = new UnZip();
		//ArrayList<String> nwList = new UnZip().UnZipgz();
		 ShapefileDataStore shpDataStore = null;  
	        int batchCount = 0;
    		for (int i = 0; i<nwList.size();i++){
    			if (nwList.get(i).indexOf("___________nw.shp") != -1){
    		        String pathString = nwList.get(i);
    		        //System.out.println("pathString============>"+pathString);
		            shpDataStore = new ShapefileDataStore(new File(pathString).toURI().toURL());  
		            shpDataStore.setCharset(Charset.forName("UTF-8"));  
		            String typeName = shpDataStore.getTypeNames()[0]; 
		            //System.out.println("typeName===>"+typeName);
		            FeatureSource<SimpleFeatureType, SimpleFeature> featureSource = null;   
